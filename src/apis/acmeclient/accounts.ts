'use strict';

import { BaseClient } from '../../client/BaseClient';

/**
 * AccountsClient
 *
 * @module OPNsense/Acmeclient/AccountsClient
 */

/**
 * Accounts
 */
export class AccountsClient extends BaseClient {
    /**
     * @openapi
     *
     * /acmeclient/accounts/add:
     *   post:
     *     summary: Add account
     *     tags:
     *       - acmeclient/accounts
     *     responses:
     *       '200':
     *         description: OK
     *         content:
     *           application/json:
     *             {}
     */
    /**
     * Add account
     *
     * @returns {Promise} Request promise
     */
    async add () {
        return this.httpClient.post('/acmeclient/accounts/add');
    }

    /**
     * @openapi
     *
     * /acmeclient/accounts/del/{uuid}:
     *   post:
     *     summary: Delete account
     *     tags:
     *       - acmeclient/accounts
     *     responses:
     *       '200':
     *         description: OK
     *         content:
     *           application/json:
     *             {}
     *     parameters:
     *       - in: path
     *         name: uuid
     *         schema:
     *           type: string
     *         required: true
     */
    /**
     * Delete account
     *
     * @param   {string}  uuid Account UUID
     * @returns {Promise}      Request promise
     */
    async del (uuid) {
        return this.httpClient.post('/acmeclient/accounts/del/' + uuid);
    }

    /**
     * @openapi
     *
     * /acmeclient/accounts/get/{uuid}:
     *   post:
     *     summary: Get account
     *     tags:
     *       - acmeclient/accounts
     *     responses:
     *       '200':
     *         description: OK
     *         content:
     *           application/json:
     *             {}
     *     parameters:
     *       - in: path
     *         name: uuid
     *         schema:
     *           type: string
     */
    /**
     * Get account
     *
     * @param   {string}  uuid Account UUID
     * @returns {Promise}      Request promise
     */
    async get (uuid = '') {
        return this.httpClient.get('/acmeclient/accounts/get/' + uuid);
    }

    /**
     * @openapi
     *
     * /acmeclient/accounts/search:
     *   get:
     *     summary: Search accounts
     *     tags:
     *       - acmeclient/accounts
     *     responses:
     *       '200':
     *         description: OK
     *         content:
     *           application/json:
     *             {}
     */
    /**
     * Search accounts
     *
     * @returns {Promise} Request promise
     */
    async search () {
        return this.httpClient.get('/acmeclient/accounts/search');
    }

    /**
     * @openapi
     *
     * /acmeclient/accounts/toggle/{uuid}/{enabled}:
     *   post:
     *     summary: Toggle account
     *     tags:
     *       - acmeclient/accounts
     *     responses:
     *       '200':
     *         description: OK
     *         content:
     *           application/json:
     *             {}
     *     parameters:
     *       - in: path
     *         name: uuid
     *         schema:
     *           type: string
     *         required: true
     *       - in: path
     *         name: enabled
     *         schema:
     *           type: string
     */
    /**
     * Toggle account
     *
     * @param   {string}       uuid    Account UUID
     * @param   {boolean|null} enabled Enabled
     * @returns {Promise}              Request promise
     */
    async toggle (uuid, enabled = null) {
        enabled = (enabled === null) ? '' : Number(Boolean(enabled));
        return this.httpClient.post('/acmeclient/accounts/toggle/' + uuid + '/' + enabled);
    }

    /**
     * @openapi
     *
     * /acmeclient/accounts/update/{uuid}:
     *   post:
     *     summary: Update account
     *     tags:
     *       - acmeclient/accounts
     *     responses:
     *       '200':
     *         description: OK
     *         content:
     *           application/json:
     *             {}
     *     parameters:
     *       - in: path
     *         name: uuid
     *         schema:
     *           type: string
     *         required: true
     */
    /**
     * Update account
     *
     * @param   {string}  uuid Account UUID
     * @returns {Promise}      Request promise
     */
    async update (uuid) {
        return this.httpClient.post('/acmeclient/accounts/update/' + uuid);
    }
}
