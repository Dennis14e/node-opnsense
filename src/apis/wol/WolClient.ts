 ...
 * Wol
 */
export class WolClient extends BaseClient {
    /**
     * @openapi
     *
     * /wol/wol/searchHost:
     ...
     */
    async searchHost () {
        return this.httpClient.get('/wol/wol/searchHost');
    }
    ...
    async wakeByUUID (uuid) {
        return this.httpClient.post('/wol/wol/set', { uuid: uuid });
    }
    ...
    async wakeByMAC (mac) {
        ...
        await this.searchHost().then(res => {
            if (res.status !== 'success') {
                return this.httpClient.response(res.status, res.result);
            }
        ...
        if (uuid === null) {
            return this.httpClient.response('error', 'No entry was found with this MAC address.');
        }
